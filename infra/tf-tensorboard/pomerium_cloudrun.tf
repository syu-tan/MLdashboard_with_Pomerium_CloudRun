  ...
  template {
    spec {
      containers {
        ...
        env {
          name = "POLICY"
          value = base64encode(templatefile("pomerium-config/policy_devel.yaml",
            { POMERIUM_URL = replace(google_cloud_run_service.tensorboard_cloudrun.status[0].url, var.dashboard_name, var.auth_name),
          DASHBOARD_URL = google_cloud_run_service.tensorboard_cloudrun.status[0].url, }))
        }
      }
    }
