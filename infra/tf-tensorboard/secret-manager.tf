...
resource "google_secret_manager_secret" "pomerium_config" {
  secret_id = var.secret_pomerium_config
...
resource "google_secret_manager_secret_version" "secret_version_pomerium_config" {
  ...
  secret_data = templatefile("pomerium-config/config_devel.yaml",
    { URL           = replace(google_cloud_run_service.tensorboard_cloudrun.status[0].url, var.dashboard_name, var.auth_name),
